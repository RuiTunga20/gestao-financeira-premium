...
class FinancialApp:
            ...
            ft.Column([
                ...
                ft.Column([
                    ...
                    ft.Row([
                        ...
                        ft.Text(f"+{self.accumulated_balance:,.0f} Kz", size=13, weight=ft.FontWeight.BOLD, color="#059669")
                    ], alignment=ft.MainAxisAlignment.SPACE_BETWEEN) if self.accumulated_balance > 0 else ft.Container(),
    ...
    def create_summary_view(self):
        ...
        spending_insights = self.create_elegant_card(
            ft.Column([
                ...
                ], spacing=8) if most_common else [ft.Text("Ainda não há padrões para analisar", size=14, color="#6B7280")]
            ])
        ) if self.expenses else ft.Container()
                ...
                ft.Row([
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Text(f"{total_saved_for_goals:,.0f}", size=20, weight=ft.FontWeight.BOLD, color="#059669"),
                            ft.Text("Kz", size=12, color="#9CA3AF")
                        ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
                        bgcolor="#ECFDF5",
                        border_radius=18,
        ...
        self.summary_view = ft.Column([
            ...
            self.create_elegant_card(
                recent_expenses_list if recent_expenses else ft.Text("Nenhuma transação registrada ainda", size=14, color="#6B7280")
            )
        ], scroll=ft.ScrollMode.AUTO)
    ...
    def setup_navigation(self):
        ...
        self.navigation_bar = ft.NavigationBar(
            destinations=[
                ft.NavigationDestination(icon=ft.icons.CREDIT_CARD_OUTLINED, label="Despesas", selected_icon=ft.icons.CREDIT_CARD),
                ft.NavigationDestination(icon=ft.icons.SAVINGS_OUTLINED, label="Metas", selected_icon=ft.icons.SAVINGS),
                ft.NavigationDestination(icon=ft.icons.ACCOUNT_BALANCE_OUTLINED, label="Renda & Dívidas", selected_icon=ft.icons.ACCOUNT_BALANCE),
                ft.NavigationDestination(icon=ft.icons.ANALYTICS_OUTLINED, label="Dashboard", selected_icon=ft.icons.ANALYTICS)
            ],
    ...
    def show_add_payment_dialog(self, goal_index):
        ...
        dialog = ft.AlertDialog(
            ...
            content=ft.Column([
                ...
                ft.Text(f"Já investido: {self.goals[goal_index].get('saved_amount', 0):,.0f} Kz", size=14, color="#059669"),
    ...
    def show_debt_confirmation_dialog(self, amount, debt_ratio):
        ...
        dialog = ft.AlertDialog(
            ...
            actions=[
                ft.TextButton("Cancelar", on_click=lambda e: setattr(dialog, 'open', False) or self.page.update()),
                ft.ElevatedButton("Continuar Mesmo Assim", on_click=lambda e: self.confirm_add_debt(dialog, amount), bgcolor=color, color="#FFFFFF")
            ]
        )
    ...
    def update_goals_list(self):
        ...
        for i, goal in enumerate(self.goals):
            ...
            remaining_months = math.ceil(remaining_cost / goal['monthly_saving']) if goal['monthly_saving'] > 0 and remaining_cost > 0 else 0
